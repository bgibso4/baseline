...
- **Small Settings/label fonts migrated from fixed `.system(size:)` to semantic styles** (e.g. `size:14`→`.subheadline`, `size:11`/`10`→`.caption`) so they scale with Dynamic Type — a real accessibility fix. Side effect: those labels (Settings rows, section headers, status badges, the Trends window-stepper label) render ~1–2pt larger than before at the default text size. Accepted as part of the Phase 2 audit work. (Applied broadly across `CadreTypography` tokens.)
- **Secondary/tertiary text colors lightened app-wide for WCAG contrast** — `textSecondary` `#797B83`→`#A0A3AA`, `textTertiary` `#7A7D86`→`#9A9DA4`. The originals measured ~4.3:1 on the translucent glass cards (just under WCAG AA 4.5:1); the lighter values clear it (~6.6–7.1:1). All de-emphasized text (section titles, tile labels, metadata, captions) is now slightly lighter. Genuine accessibility fix, kept after reviewing live on simulator.

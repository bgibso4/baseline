...
/// Combined review and manual scan-entry form. Both modes share state
/// (date picker, overwrite alert, weight unit, focused field) and most
/// rendering (date chip, fields, save button), so they live in one struct;
/// `mode` switches between them at the body level.
///
/// `mode == .review` is the post-OCR review step — adds warning, single-photo
/// and retry banners on top of the standard fields. `mode == .manual` is the
/// no-OCR manual entry step — Cancel-style header, no banners.
struct ScanReviewForm: View {
    enum Mode {
        case review
        case manual
    }
    ...
    let mode: Mode
    /// Dismisses the entire scan-entry flow. Wired from `ScanEntryFlow`'s
    /// `@Environment(\.dismiss)` so save / cancel actions can close the sheet.
    let onClose: () -> Void
    /// Threaded focus state — the gradient background on `ScanEntryFlow` also
    /// dismisses focus on tap, so the focus binding has to live on the parent.
